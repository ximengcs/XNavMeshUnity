...
namespace XFrame.PathFinding
{
    ...
    public static class EarClippingHoleMethods
    {
        ...
        public static List<XVector2> MergeHolesWithHull(List<XVector2> verticesHull, List<List<XVector2>> allHoleVertices)
        {
            //Validate
            ...
            EarClippingPolygon hull = new EarClippingPolygon(new Polygon(verticesHull));

            List<EarClippingPolygon> holes = new List<EarClippingPolygon>();

            int counter = 1;

            foreach (List<XVector2> hole in allHoleVertices)
            {
                //Validate data
                ...
                EarClippingPolygon holePolygon = new EarClippingPolygon(new Polygon(hole));
        ...
        private static void MergeHoleWithHull(EarClippingPolygon hull, EarClippingPolygon hole)
        {
            ...
            XVector2 lineStart = hole.maxX_Vert;
            //Just add some value so we know we are outside
            XVector2 lineEnd = new XVector2(hull.maxX_Vert.X + 0.01f, hole.maxX_Vert.Y);
            //Important to add lineStart first because we will need it later
            Edge line_hole_to_outside = new Edge(lineStart, lineEnd);
            ...
            List<XVector2> verticesHull = hull.Vertices;
        ...
        private static void FindVisibleVertexOnHUll(EarClippingPolygon hull, EarClippingPolygon hole, Edge line_hole_to_outside, out int closestEdge, out XVector2 visibleVertex)
        {
            //The first and second point on the hull is defined as edge 0, and so on...
            closestEdge = -1;
            //The vertex that should be visible to the hole (which is the max of the line that's intersecting with the line)
            visibleVertex = new XVector2(-1f, -1f);


            //Do line-line intersection to find intersectionVertex which is the point of intersection that's the closest to the hole
            XVector2 intersectionVertex = new XVector2(-1f, -1f);
            ...
            {
                XVector2 p1_hull = verticesHull[i];
                XVector2 p2_hull = verticesHull[XMath.ClampListIndex(i + 1, verticesHull.Count)];
                ...
                Edge line_hull = new Edge(p1_hull, p2_hull);

                bool isIntersecting = XMath.LineLine(line_hole_to_outside, line_hull, includeEndPoints: true);

                //Here we can maybe add a check if any of the vertices is on the line???

                if (isIntersecting)
                {
                    XVector2 testIntersectionVertex = XMath.GetLineLineIntersectionPoint(line_hole_to_outside, line_hull);

                    //if (hole.id == 3) TestAlgorithmsHelpMethods.DebugDrawCircle(testIntersectionVertex.ToVector3(), 0.2f, Color.green);

                    float distanceSqr = XVector2.SqrDistance(line_hole_to_outside.P1, testIntersectionVertex);
            ...
            XVector2 p1 = hull.Vertices[closestEdge];
            XVector2 p2 = hull.Vertices[XMath.ClampListIndex(closestEdge + 1, hull.Vertices.Count)];

            visibleVertex = p1;

            //They are the same so pick the one that is the closest
            if (Mathf.Abs(p1.X - p2.X) < XMath.EPSILON)
            {
                float hole_p1 = XVector2.SqrDistance(hole.maxX_Vert, p1);
                float hole_p2 = XVector2.SqrDistance(hole.maxX_Vert, p2);
        ...
        private static void FindActualVisibleVertexOnHull(EarClippingPolygon hull, EarClippingPolygon hole, XVector2 intersectionVertex, ref XVector2 visibleVertex)
        {
            //Form a triangle
            Triangle t = new Triangle(hole.maxX_Vert, intersectionVertex, visibleVertex);
            ...
            List<XVector2> reflectVertices = FindReflectVertices(hull, hole);
            ...
            foreach (XVector2 v in reflectVertices)
            {
                if (XMath.PointTriangle(t, v, includeBorder: true))
                {
                    ...
                    if (angle < minAngle)
                    {
                        ...
                        minDistSqr = XVector2.SqrDistance(v, hole.maxX_Vert);
                    ...
                    else if (Mathf.Abs(angle - minAngle) < XMath.EPSILON)
                    {
                        float distSqr = XVector2.SqrDistance(v, hole.maxX_Vert);
        ...
        private static List<XVector2> FindReflectVertices(EarClippingPolygon hull, EarClippingPolygon hole)
        {
            List<XVector2> reflectVertices = new List<XVector2>();
            ...
            for (int i = 0; i < verticesHull.Count; i++)
            {
                ...
                XVector2 p_prev = verticesHull[XMath.ClampListIndex(i - 1, verticesHull.Count)];

                XVector2 p_next = verticesHull[XMath.ClampListIndex(i + 1, verticesHull.Count)];
